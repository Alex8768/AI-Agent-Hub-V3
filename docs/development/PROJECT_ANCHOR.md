...
Continue extraction-only convergence of answer/reasoning facades after A2.74 closure,
reducing residual high-density helper concentration while preserving runtime parity.

### Why Now

A2.74 closed with extraction and guardrail recalibration to `2100`/`313` baselines.
Residual hotspots remain in answer/reasoning facades, requiring another bounded
inventory-first extraction cycle.

### Architecture Position

Target A2.75 boundaries:

- **Answer convergence phase 19**
...
- **Reasoning convergence phase 19**
...
- no-growth baseline inventory captured:
  - `src/services/answer/answer_service.py`: `2100` lines
  - `src/layers/pro/reasoning/engine.py`: `313` lines
- hotspot inventory captured for extraction planning:
  - answer clusters: `_apply_diagnostics`, `_build_feedback_adaptation_bundle`,
...
#### Patch 2 - Answer extraction phase-19
...
Patch 2 artifacts:
- feedback adaptation builder seam extracted into
  `src/services/answer/reasoning/llm_planner_policy.py`:
  - `build_feedback_adaptation_bundle`
- `src/services/answer/answer_service.py` now retains thin compatibility wrapper:
  - `_build_feedback_adaptation_bundle` delegates to extracted policy seam
- facade reduction achieved:
  - `src/services/answer/answer_service.py`: `2100 -> 2051` lines
...
#### Patch 3 - Reasoning extraction phase-19
...
Patch 3 artifacts:
- fallback orchestration seam extracted into
  `src/layers/pro/reasoning/evaluation/runtime_productization.py`:
  - `synthesize_fallback_with_runtime_settings`
- `src/layers/pro/reasoning/engine.py` now retains thin compatibility wrapper:
  - `_synthesize_fallback` delegates to extracted runtime-productization seam
- facade reduction achieved:
  - `src/layers/pro/reasoning/engine.py`: `313 -> 310` lines
...
Patch 4 artifacts:
- quality-gate no-growth baseline recalibration applied in
  `tests/unit/services/answer/test_answer_orchestration_quality_gate.py`:
  - `answer_service_max_lines`: `2100 -> 2051`
  - `reasoning_engine_max_lines`: `313 -> 310`
- import-budget verification (no-growth, unchanged):
  - answer local imports: `15` (budget `<= 15`)
  - reasoning local imports: `14` (budget `<= 17`)
- guardrail checks green:
  - `tests/unit/services/answer/test_answer_orchestration_quality_gate.py`
  - result: `16 passed`
...
- No net-new user-facing features during A2.75
...
A2.75 is complete when:

- answer and reasoning convergence phase-19 extraction is completed with parity
