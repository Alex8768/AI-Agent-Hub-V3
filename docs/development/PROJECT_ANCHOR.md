...
Continue extraction-only convergence of answer/reasoning facades after A2.76 closure,
reducing residual high-density helper concentration while preserving runtime parity.

### Why Now

A2.76 closed with extraction and guardrail recalibration to `2000`/`310` baselines.
Residual hotspots remain in answer/reasoning facades, requiring another bounded
inventory-first extraction cycle.

### Architecture Position

Target A2.77 boundaries:

- **Answer convergence phase 21**
...
- **Reasoning convergence phase 21**
...
- no-growth baseline inventory captured:
  - `src/services/answer/answer_service.py`: `2000` lines
...
- hotspot inventory captured for extraction planning:
  - answer clusters: `_apply_diagnostics`, `retrieve`,
    `_build_conversational_runtime_parity_bundle`, `handle_contract`, `_infer_assistant_intent`
...
#### Patch 2 - Answer extraction phase-21
...
Patch 2 artifacts:
- assistant intent inference seam extracted into
  `src/services/answer/reasoning/llm_planner_policy.py`:
  - `infer_assistant_intent`
- `src/services/answer/answer_service.py` now retains thin compatibility wrapper:
  - `_infer_assistant_intent` delegates to extracted policy seam
- facade reduction achieved:
  - `src/services/answer/answer_service.py`: `2000 -> 1953` lines
...
- quality-gate no-growth baseline recalibration applied in
  `tests/unit/services/answer/test_answer_orchestration_quality_gate.py`:
  - `answer_service_max_lines`: `2000 -> 1953`
  - `reasoning_engine_max_lines`: `310 -> 292`
...
- run focused and full-suite checks and close A2.77 with docs sync.
...
- No net-new user-facing features during A2.77
...
A2.77 is complete when:

- answer and reasoning convergence phase-21 extraction is completed with parity
...
A2.76 complete - Facade Convergence Phase 20 closed with extraction + guardrails + parity.
